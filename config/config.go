...
func configDefaults() map[string]any {
	return map[string]any{
		"server":       "https://duskjo.nanoteck137.net",
		"token":        "",
		"github_token": "",
	}
}
...
func readConfigFromEnv() map[string]any {
	prefix := strings.ToUpper(dusk.AppName + "_")
...
func DefaultConfigDir() (string, error) {
	...
	return filepath.Join(userDir, dusk.AppName), nil
}
...
func Load(cfgFile string) (*Config, error) {
	...
	if cfgFile != "" {
		...
		paths = append(paths, filepath.Join("/etc", dusk.AppName, "config.toml"))

		if userDir, err := os.UserConfigDir(); err == nil {
			paths = append(paths, filepath.Join(userDir, dusk.AppName, "config.toml"))
		}
...
func ConfigPath(cfgFile string) (string, error) {
	if cfgFile != "" {
		return cfgFile, nil
	}

	if userDir, err := os.UserConfigDir(); err == nil {
		return filepath.Join(userDir, dusk.AppName, "config.toml"), nil
	}

	return "config.toml", nil
}

func Save(cfgFile string, config *Config) error {
	path, err := ConfigPath(cfgFile)
	if err != nil {
